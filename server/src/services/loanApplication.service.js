...
export const loanApplicationService = {
  ...
  async updateDraft(userId, loanApplicationId, data) {
    ...
    if (data.currentStep !== undefined) {
      ...
      updateData.currentStep =
        loanValidationService.assertAllowedCurrentStep(
    ...
    if (amountChanged || tenureChanged) {
      if (nextAmount === null || nextTenure === null) {
        ...
        loanValidationService.validateLoanAmount(
          configuration,
          nextAmount,
        );

        loanValidationService.validateTenure(
          configuration,
          nextTenure,
        );
    ...
    return loanApplicationRepository.transaction(async (tx) => {
      return loanApplicationRepository.update(
        tx,
        application.id,
        updateData,
      );
  ...
  async submit(userId, loanApplicationId) {
    const application =
      await loanApplicationRepository.findById(loanApplicationId);
    ...
    if (application.status !== "DRAFT") {
      throw new AppError(
        "Only draft loan applications can be submitted",
        400,
      );
    }
    ...
    const eligibilityRule =
      await loanApplicationRepository.findEligibilityRule(
        application.loanTypeId,
      );
    ...
    return loanApplicationRepository.transaction(async (tx) => {
       ...
       */
      const current =
        await tx.loanApplication.findUnique({
          where: {
            id: application.id,
          },
        });
      ...
      if (current.status !== "DRAFT") {
        throw new AppError(
          "Loan application has already been submitted",
          400,
        );
      }
