...
function LoanDetailsStep({
  ...
  const loanTypes = useMemo(() => {
    ...
    return types.filter(
      (loan) =>
        loan.code !== "STUDENT_LOAN" && loan.code !== "EDUCATION_LOAN",
    );
  }, [data?.data]);
  ...
  const isSchoolLoanSelected = selectedLoan?.code === "SCHOOL_STUDENT_LOAN";

  useEffect(() => {
    if (isEditMode || loanTypeId || loanTypes.length !== 1) {
      return;
    }

    setValue("loanTypeId", loanTypes[0].id, { shouldValidate: true });
  }, [isEditMode, loanTypeId, loanTypes, setValue]);

  useEffect(() => {
    if (isSchoolLoanSelected) {
      setValue("loanAmount", undefined, { shouldValidate: false });
      setValue("tenureMonths", undefined, { shouldValidate: false });
    }
  }, [isSchoolLoanSelected, setValue]);
  ...
  return (
    <form onSubmit={handleSubmit(submit)} noValidate>
      <LoanWizardShell
        ...
        description={
          isEditMode
            ...
            : isSchoolLoanSelected
              ? "Confirm School Student Loan to start the 7-step application."
      ...
      >
        ...
        {selectedLoan && (
          <section className="space-y-4 rounded-lg border bg-muted/20 p-4 sm:p-5">
        ...
        <FormField
          ...
          required
          className={isSchoolLoanSelected ? "hidden" : undefined}
          helperText={
        ...
        {isSchoolLoanSelected ? (
          <p className="text-helper rounded-lg border border-primary/20 bg-primary/5 p-4">
            You'll enter the exact loan amount and tenure later, in the "Loan
            Requirement" step, after student, co-applicant, income, and
            existing-loan details have been collected.
          </p>
