...
function EducationDetailsStep({ loanApplicationId, onBack, onNext }) {
  ...
  const [createStudentLoanDetails, { isLoading: isCreating }] =
    useCreateStudentLoanDetailsMutation();
  ...
  async function onSubmit(values) {
    ...
    try {
      if (hasExistingDetails) {
        ...
        const response = await createStudentLoanDetails({
          loanApplicationId,
          data: payload,
        }).unwrap();
  ...
  return (
    <form onSubmit={handleSubmit(onSubmit)} noValidate>
      <LoanWizardShell
        step={2}
        title="Student Details"
        description="Provide the student and course information for this student loan."
      >
