...
import { useGetEducationLoanDetailsQuery } from "../api/educationLoanApi";
...
function ReviewSubmitStep({ loanApplication, loanApplicationId, onBack }) {
  ...
  if (isLoanLoading || isEducationLoading || isDocumentsLoading) {
    return (
      <LoanWizardShell step={5} title="Review Application" description="Loading...">
  ...
  return (
    <LoanWizardShell
      ...
      description="Review all your information before submitting the application."
    >
        ...
        {!educationDetails?.parents?.length ? (
          ...
          <div className="space-y-6">
            {educationDetails.parents.map((parent) => (
              <div key={parent.id} className="space-y-4 border-t pt-6 first:border-t-0 first:pt-0">
      ...
      {submittedApplication ? (
        <section className="space-y-4 rounded-xl border border-[hsl(var(--success-border))] bg-[hsl(var(--success-soft))] p-5">
          <div>
